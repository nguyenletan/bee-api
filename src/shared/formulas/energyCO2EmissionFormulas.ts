...
export class EnergyCO2EmissionFormulas {
  ...
  static calculateC02EmissionForEachSystem(
    energyConsumption: number,
    countryCode: string,
  ): number {
    const countryGridEmissionReference = CountryGridEmissionReference.find(
      (x) => x.countryCode === countryCode,
    );

    const gridEmissionRate = countryGridEmissionReference
      ? countryGridEmissionReference.CO2Emissions
      : CountryGridEmissionReference[0].CO2Emissions;
